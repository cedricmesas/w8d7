...
nouveau.addEventListener('click', () => {
    do {
        numberRandom = genNumInt(citations.length);
    } while (numberRandom == last)

    citation.textContent = citations[numberRandom][0];
    auteur.textContent   = citations[numberRandom][1];
    last                 = numberRandom;
});